...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (user.role === Role.GURU && (!user.kelasId || kelasId !== user.kelasId)) {
      return NextResponse.json({ message: "Guru hanya boleh mencatat kegiatan untuk kelas yang ditugaskan" }, { status: 403 });
    }
